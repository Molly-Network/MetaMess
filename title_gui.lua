...
surface.CreateFont("PT_Subheader", {
...
surface.CreateFont("PT_SmallButton", {
    font = "Segoe UI Semilight",
    extended = false,
    size = 16,
    weight = 500,
...
local function StyleButton(Perent, Icon, Text, ColorNormal, ColorHovered)
    ...
    Button.Paint = function(self, w, h)
        ...
        --Text
        draw.SimpleText(Text, "PT_Button", w - 40, h / 2, ColorWhite, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    end
...
local function SmallButton(Perent, Text, ColorNormal, ColorHovered)
    ...
    ButtonMain.Paint = function(self, w, h)
        ...
        --Text
        draw.SimpleText(Text, "PT_SmallButton", w / 2, h / 2, ColorWhite, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    end
...
local function StyleTextEntry(Perent, Width, Placeholder)
    ...
    TextEntry:SetFont("PT_Subheader")
...
local function PT_Note(parent, text, color)
    ...
    Notification.Paint = function(self, w, h)
        ...
        draw.DrawText(text, "PT_Button", w / 2, h / 2 - 13, ColorWhite, TEXT_ALIGN_CENTER)
    end
...
concommand.Add("title_gui", function()
    ...
    LoginPanel.Paint = function(self, w, h)
        if EditMode then
            draw.DrawText("Editing Line " .. ActiveLine, "PT_Header2", 10, -4, ColorWhite)
            draw.DrawText("Use the options to edit the title", "PT_Button", 10, 22, ColorOffGrey)
        else
            draw.DrawText("Add a Title", "PT_Header2", 10, -4, ColorWhite)
            draw.DrawText("Use the options to create a title", "PT_Button", 10, 22, ColorOffGrey)
        end
    end
    ...
    DelayMain.Paint = function(self, w, h)
        draw.RoundedBox(10, 0, 0, w, h, ColorLightDark)
        draw.SimpleText("Delay", "PT_Button", w - 10, h / 2, ColorOffGrey, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
    end
    ...
    ColorSelect.Paint = function(self, w, h)
        ...
        draw.SimpleText("Colour", "PT_Button", w - 40, h / 2, ColorWhite, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    end
    ...
    local function MoveTitle(Current, Wanted)
        if Wanted > #Titles or Wanted < 1 then
            PT_Note(LoginPanel, "Invalid Position", ColorRed)
        else
    ...
    Clear.DoClick = function()
        ...
        Yes.DoClick = function()
            ...
            SaveTable("meta_titles_cache/autoload.json")
            ClearForm()
            PT_Note(LoginPanel, "Table Cleared", ColorBlue)
            BaseFrame:Close()
        end
    end
    ...
    TitleList.OnRowRightClick = function(ID, Line)
        ...
        local Copy = Menu:AddOption("Copy", function()
            CopyTitle(ID, Line)
        end)

        Copy:SetIcon("icon16/page_copy.png")

        local Edit = Menu:AddOption("Edit", function()
            EditTitle(ID, Line)
        end)
    ...
    local function SaveFunc()
        if DelayEntry:GetValue() == "" or TextEntry:GetValue() == "" then
            PT_Note(LoginPanel, "Invalid Input", ColorRed)
        else
            if tonumber(DelayEntry:GetValue()) < 0 then
                PT_Note(LoginPanel, "Delay lower than 0", ColorRed)
            else
                if EditMode then
                    table.remove(Titles, ActiveLine)

                    table.insert(Titles, ActiveLine, {
                        [1] = TextEntry:GetValue(),
                        [2] = DelayEntry:GetValue()
                    })
                ...
                PT_Note(LoginPanel, "Added/Edited", ColorGreen)
                ClearForm()
                EditMode = false
            end
        end
    ...
    SetChanges.Paint = function(self, w, h)
        ...
        --Text
        if EditMode then
            draw.SimpleText("Edit", "PT_Button", w - 40, h / 2, ColorWhite, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        else
            draw.SimpleText("Add", "PT_Button", w - 40, h / 2, ColorWhite, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        end
    end
    ...
    Delete.DoClick = function()
        if EditMode then
            table.remove(Titles, ActiveLine)
            ClearForm()
            PT_Note(LoginPanel, "Title Deleted", ColorBlue)
        else
            PT_Note(LoginPanel, "No Title Selected", ColorRed)
        end
    end
...
list.Set("DesktopWindows", "Ttile GUI", {
    title = "Title GUI",
    icon = icon,
